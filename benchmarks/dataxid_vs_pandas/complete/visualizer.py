"""Grafik — DataXID vs Pandas COMPLETE Mode Hız + RAM Karşılaştırması"""
...
def draw_charts(row_counts, dataxid_times, pandas_times, dataxid_mems, pandas_mems):
    _draw_speed(row_counts, dataxid_times, pandas_times)
    _draw_memory(row_counts, dataxid_mems, pandas_mems)


def _draw_speed(row_counts, dataxid_times, pandas_times):
    ...
    ax.set_title("DataXID vs Pandas — Complete Mode Hız (10 Kolon)", fontweight="bold", pad=20, fontsize=14)
    ax.set_xticks(x)
...
def _draw_memory(row_counts, dataxid_mems, pandas_mems):
    ...
    fig, ax = plt.subplots(figsize=(11, 6))
    rects1 = ax.bar(x - width / 2, dataxid_mems, width, label="DataXID (Polars)", color="#70AD47")
    rects2 = ax.bar(x + width / 2, pandas_mems, width, label="Pandas (YData)", color="#FFC000")

    ax.set_ylabel("Ekstra RAM (MB)", fontweight="bold")
    ax.set_xlabel("Satır Sayısı", fontweight="bold")
    ax.set_title("DataXID vs Pandas — Complete Mode RAM Karşılaştırması (10 Kolon)", fontweight="bold", pad=20, fontsize=14)
    ax.set_xticks(x)
    ax.set_xticklabels(labels)
    ax.legend(loc="upper left", frameon=False)
    ax.grid(axis="y", linestyle="-", alpha=0.3)
