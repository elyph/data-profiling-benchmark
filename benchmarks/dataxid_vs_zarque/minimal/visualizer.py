"""Grafik — DataXID vs Zarque Hız Karşılaştırması"""
...
def draw_comparison_chart(row_counts, dataxid_times, zarque_times):
    print("\n[+] Hız grafiği çiziliyor...")
    ...
    ax.set_title("DataXID vs Zarque — Hız Karşılaştırması (10 Kolon)", fontweight="bold", pad=20, fontsize=14)
    ax.set_xticks(x)
    ...
    plt.figtext(0.12, 0.02, f"Environment: {cpu}", ha="left", fontsize=9)
    plt.subplots_adjust(bottom=0.18)
    plt.savefig("benchmark_speed.png", dpi=300)
    plt.close()
    print("    benchmark_speed.png kaydedildi.")
